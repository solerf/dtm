...
func collectLinksFromMappings(mappings LocalMappings) map[string]struct{} {
...
func getMappings() (LocalMappings, error) {
	homeDir, err := os.UserHomeDir()
	if err != nil {
		return LocalMappings{}, fmt.Errorf("get homedir: %w", err)
	}

	mappingsPath := filepath.Join(homeDir, dtmMappingsFileName)
	file, err := os.ReadFile(mappingsPath)
	if err != nil {
		return LocalMappings{}, fmt.Errorf("reading mappings in [%s]: %w", homeDir, err)
	}

	var mappings LocalMappings
	err = json.Unmarshal(file, &mappings)
	if err != nil {
		return LocalMappings{}, fmt.Errorf("deserialize mappings in [%s]: %w", homeDir, err)
	}
	return mappings, nil
}
